...
function addRow(data) {
    ...
    let query = global.mysql.format(insertQuery,["Auditors",]);
...
function updateRow(data) {
    ...
    let query = global.mysql.format(updateQuery,["Auditors","name",data.name,"auditor_id",data.auditor_id]);
    connect.con.query(query,(err, response) => {
        if(err) {
            console.error(err);
            return;
...
function queryRow(auditor) {
    let selectQuery = "SELECT name FROM Auditors WHERE auditor_id = 1";
    let query = global.mysql.format(selectQuery,["Auditors","auditor_id", auditor]);
...
function deleteRow(auditor) {
    ...
    let query = global.mysql.format(deleteQuery, ["Auditors", "auditor_id", auditor]);
    connect.con.query(query,(err, response) => {
        if(err) {
            console.error(err);
            return;
